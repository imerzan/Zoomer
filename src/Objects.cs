...
namespace Zoomer
{
    public class ZoomerConfig : IDisposable
    {
        ...
        private void SetState() // Sets Client/Server State
        {
            try
            {
                this.Server?.Dispose(); // Stop existing server (if any)
        ...
        public void Install(string ip, string port, string mac, OperatingMode mode, Dictionary<byte, Hotkey> editorhotkeys) // Apply current configuration and write to Registry.
        {
            try
            {
                this.IpAddress = ip;
        ...
        public void Uninstall() // Remove current configuration, and delete from Registry.
        {
            try
            {
    ...
    public class ZoomerClient
    {
        ...
        public void Transmit(byte[] actionkey) // Client Mode
        {
            try
            {
                using (Socket client = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp))
                {
                    client.SendTo(Crypt.Encrypt(actionkey), this.Endpoint); // Transmits ActionKey byte array to specified IP Address & Port via UDP Datagram.
                    client.Close();
                }
            }
            catch { }
    ...
    public class ZoomerServer : IDisposable
    {
        ...
        private void Listen() // Server Mode
        {
            ...
            while (true) // Main loop
            {
                ...
                {
                    int key;
                    byte[] buffer = this.Server.Receive(ref this.Endpoint); // Listen on bound port for UDP Datagrams
                    if (int.TryParse(Encoding.ASCII.GetString(Crypt.Decrypt(buffer)), out key)) // Make sure received data is valid ActionKey
                    {
                        Win32API.SendKey((byte)key); // Send ActionKey keypress as requested by the client, 50ms key delay
                    }
                }
    ...
    public class WakeOnLan
    {
        ...
        {
            this.EP = new IPEndPoint(IPAddress.Broadcast, 9); // Port 9 most common WOL port
            this.MP = this.BuildMagicPacket(mac);
        }
        public void Send() // Broadcasts Magic Packet based on provided MAC Address to UDP Port 9 to wake computer(s) from sleep.
        {
            try
            {
                if (this.MP == null) throw new NullReferenceException("Magic Packet is null. Verify MAC Address is entered correctly.");
                using (Socket sock = new Socket(AddressFamily.InterNetwork, SocketType.Dgram, ProtocolType.Udp))
                {
                    sock.EnableBroadcast = true; // Enable broadcast
                    sock.SendTo(this.MP, this.EP); // Transmit Magic Packet on Port 9
